...
enumError wdf2iso ( ccp fname )
{
    ...
    GenImageFileName(&fo.f,dest,outname,OFT_PLAIN);
    ...
    if (opt_split)
	SetupSplitFile(&fo.f,OFT_PLAIN,opt_split_size);

    fo.file_size = fi.file_size;
    err = SetupWriteSF(&fo,OFT_PLAIN);
    if (err)
	goto abort;
...
{
    SetupLib(argc,argv,NAME,PROG_WDF2ISO);
    ...
    {
	const int last_err = wdf2iso(argv[i]);
	if ( max_err < last_err )
	    max_err = last_err;
    }
