...
static const char help_text[] =
    "\n"
    TITLE "\n"
    "This tool converts files into WDF files.\n"
    "\n"
    ...
    "   -t --test            Run in test mode, don't create a WDF file.\n"
    "\n"
...
enumError iso2wdf ( ccp fname )
{
    ...
    ccp outname = fi.f.outname ? fi.f.outname : fname;
    GenImageFileName(&fo.f,dest,outname,OFT_WDF);
    ...
    if (opt_split)
	SetupSplitFile(&fo.f,OFT_WDF,opt_split_size);

    fo.file_size = fi.file_size;
    err = SetupWriteSF(&fo,OFT_WDF);
    if (err)
	goto abort;
...
{
    SetupLib(argc,argv,NAME,PROG_ISO2WDF);
    ...
    {
	const int last_err = iso2wdf(argv[i]);
	if ( max_err < last_err )
	    max_err = last_err;
    }
